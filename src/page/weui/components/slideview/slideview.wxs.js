...
var touchmove = function(event, ownerInstance) {
  ...
  if (st.out) {
    ...
    for (; i >= 0; i--) {
      ...
      var transformx =
        st.size.buttons[i].max -
        Math.min(st.size.buttons[i].max, transform + transformTotal)
      btns[i].setStyle({
...
var touchend = function(event, ownerInstance) {
  ...
  if (
    Math.abs(event.changedTouches[0].pageX - st.startX) < st.throttle ||
    event.changedTouches[0].pageX - st.startX > 0
  ) {
    // 方向也要控制
    st.out = false
    ins.setStyle({
...
var showButtons = function(ins, ownerInstance, withDuration) {
  ...
  for (; i >= 0; i--) {
    ...
    btns[i].setStyle({
      ...
      transition:
        'transform ' +
        (withDuration ? withDuration + rebounceTime : withDuration) +
        's'
    })
    st.size.buttons[i].transformx = transformx
    transformTotal += transform
